...
import faiss
from transformers import DPRContextEncoder, DPRContextEncoderTokenizer, HfArgumentParser
...
def embed(documents: dict, ctx_encoder: DPRContextEncoder, ctx_tokenizer: DPRContextEncoderTokenizer) -> dict:
    ...
    inputs = [' '.join(['<BOK>', doc, '<EOK>']) for doc in documents["text"]]
    input_ids = ctx_tokenizer(
        inputs, truncation=True, padding="longest", return_tensors="pt"
    )["input_ids"]
...
def main(
    ...
    logging.info(f'Using custom model {rag_example_args.dpr_ctx_encoder_model_name}')
    logging.info(f'Using custom tokenizer {rag_example_args.dpr_ctx_encoder_tokenizer_name}')
...
class RagExampleArguments:
    ...
    dpr_ctx_encoder_tokenizer_name: str = field(
        default="facebook/dpr-ctx_encoder-multiset-base",
        metadata={
            "help": "The DPR context encoder tokenizer to use. Either 'facebook/dpr-ctx_encoder-single-nq-base' or 'facebook/dpr-ctx_encoder-multiset-base'"
        },
    )
