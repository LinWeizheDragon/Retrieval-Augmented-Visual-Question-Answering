...
class T5ExecutorWithKnowledge(BaseExecutor):
    ...
    def _generative_step(self, sample_batched, batch_idx):
        ...
        # Aggregate scores
        score_matrix = F.softmax(torch.stack(outputs.scores), dim=-1)
        sequence_matrix = outputs.sequences[:, 1:] # batch_size x seq_len
        score_matrix = score_matrix.permute(1,0,2) # Batch_size x seq_len x token_num
        scores = torch.zeros(score_matrix.shape[0], score_matrix.shape[1])
        for batch_index in range(score_matrix.shape[0]):
            for token_index in range(score_matrix.shape[1]):
                token = sequence_matrix[batch_index, token_index]
                if token in [self.decoder_tokenizer.pad_token_id, self.decoder_tokenizer.eos_token_id]:
                    scores[batch_index, token_index] = 0
                    break
                else:
                    scores[batch_index, token_index] = score_matrix[batch_index, token_index, token]
        
        # Non-zero mean of scores!
        mask = scores!=0
        # print(scores.sum(dim=-1), mask.sum(dim=-1))
        sequence_scores = scores.sum(dim=-1)/mask.sum(dim=-1)
        ...
        for index, i in enumerate(labels):
            ...
            output_sequence = outputs.sequences[index]
            ...
            predictions.append({
                ...
                'score': sequence_scores[index].item(),
            })

            item = self.data_loader.data.vqa_data_with_dpr_output.lookup[str(question_id)]
            table_entry = [
                question_id,
                item['img_key'],
                item['question'],
                ...
                item['gold_answer'],
                decoded_output,
                item['passage_content'],
                sequence_scores[index].item(),
            ]
    ...
    def evaluate_outputs(self, step_outputs, mode='test'):
        ...
        columns=["question_id", "image_key", "question", "caption", "answers", "gold_answer", "prediction", "passage_content", "score"]
        ...
        # There are multiple answers for a question, so sort them
        questionId2answers = {}
        for answer in batch_predictions:
            question_id = answer['question_id']
            questionId2answers.setdefault(question_id, [])
            questionId2answers[question_id].append((answer['answer'], answer['score']))
        
        batch_predictions = [] # reset predictions

        def most_frequent(List):
            return max(set(List), key = List.count)

        for question_id, answer_list in questionId2answers.items():
            answer_text_list = [item[0] for item in answer_list]
            answer_score_list = [item[1] for item in answer_list]
            answer2score = {}
            answer_count = {}
            for item in answer_list:
                answer2score.setdefault(item[0], 0)
                answer2score[item[0]] += item[1]
                answer_count.setdefault(item[0], 0)
                answer_count[item[0]] += 1
