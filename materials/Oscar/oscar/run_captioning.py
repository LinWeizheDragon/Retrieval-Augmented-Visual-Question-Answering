...
from transformers2.pytorch_transformers import BertTokenizer, BertConfig
from transformers2.pytorch_transformers import AdamW, WarmupLinearSchedule, WarmupConstantSchedule
...
def test(args, test_dataloader, model, tokenizer, predict_file):
    ...
    res_data = {}
    for img_key, res in gen_rows():
        res_data[img_key] = res

    with open(args.output_prediction_path, 'w') as f:
        json.dump(res_data, f, indent=4)
    
    input('predictions have been saved. done. can ctrl+c to exit now. by Weizhe')
...
def main():
    ...
    parser.add_argument("--output_prediction_path", type=str, default='', 
